...
def find_closest_speech_lengths(target_count, max_file_count=None, visualize=False):
    ...
    for filename, duration in target_noisy_files:
        noisy_samples[i], _ = librosa.load(os.path.join(noisy_trainset_28spk_directory, filename))
        clean_samples[i], _ = librosa.load(os.path.join(clean_trainset_28spk_directory, filename))
        i += 1
